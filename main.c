...
void drawCard(int x, int y, Card card) {
    ...
    gfx_text(x + 5, y + 15, card.faceValue);
    gfx_text(x + 5, y + 30, card.suit);
}
...
void determineWinner() {
    if (playerTotal > 21) {
        gfx_text(50, WINDOW_HEIGHT / 2, "Player busts! Dealer wins.");
    } else if (dealerTotal > 21) {
        gfx_text(50, WINDOW_HEIGHT / 2, "Dealer busts! Player wins.");
    } else if (playerTotal > dealerTotal) {
        gfx_text(50, WINDOW_HEIGHT / 2, "Player wins!");
    } else if (dealerTotal > playerTotal) {
        gfx_text(50, WINDOW_HEIGHT / 2, "Dealer wins!");
    } else {
        gfx_text(50, WINDOW_HEIGHT / 2, "It's a draw!");
    }
    gfx_flush();
    sleep(3);
    resetGame();
